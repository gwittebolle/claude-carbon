...
The `cost_usd` column is the theoretical API list value of the usage (what it would cost on pay-as-you-go), not the subscription price actually paid. It uses current Anthropic list pricing per million tokens (re-read 2026-09-07): Opus 4.6+ at $5 input / $25 output (not the retired $15/$75 of Opus 4.0/4.1), Sonnet at $2/$10 (the Claude Sonnet 5 introductory price since 2026-06-30, made permanent; Sonnet 4.6 and 4.5 still list $3/$15, and since prices are per family a Sonnet 4.6 session is under-billed by a third), Haiku at $1/$5, Fable 5 and 5.1 at $10/$50. Cache read is billed at 0.1x input; Fable 5.1 and Mythos 5.1 bill it at 0.025x, which this plugin does not model yet (single multiplier), so their cache reads are over-billed 4x. Cache writes are billed per TTL tier, 1.25x input for the 5-minute tier and 2x for the 1-hour tier, and the tier is not assumed: each session's split is read from `usage.cache_creation.ephemeral_1h_input_tokens` and stored alongside the total, so a session mixing both tiers is priced correctly.
