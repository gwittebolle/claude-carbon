...
Claude Code can be pointed at non-Anthropic models (e.g. local models behind `ANTHROPIC_BASE_URL`). Their impact profile is not an AWS datacenter's, so neither the emission factors nor the API pricing apply. Sessions whose dominant model string does not contain `claude` (including the `<synthetic>` marker) are stored with their raw token counts but `cost_usd = 0`, `co2_grams = 0` and `excluded = 1`, and are left out of all report aggregates. Additional models can be excluded by name via the `exclude_models` patterns in `data/factors.json`. Because raw tokens are preserved, excluded sessions can be re-priced later by `recompute.sh` if factors for local models are ever added. The same rule applies per model inside a session: a subagent running on a local model keeps its tokens in its `session_models` row at `co2_grams = 0` and `cost_usd = 0`, while the session's Claude models are counted normally.
...
Each deduplicated message is counted under the model that produced it (`message.model`), and a session's main transcript and every subagent transcript (`<session>/subagents/*.jsonl`) are merged per model. A session where the main thread ran on Opus 5.5, a `/model` switch sent a few turns to Sonnet 4.6 and subagents ran on Haiku 4.5 is stored as three rows of the `session_models` table, each priced and converted at its own model; the `sessions` row keeps the totals and the main thread's dominant model, and its `co2_grams` and `cost_usd` are the sum over those rows. A message without a model string is counted under its file's dominant model. Until September 2026 the split was per file: each file was priced at its dominant model, which is the same thing unless a model switch happened inside one file.

## Surviving the 30-day transcript purge

Claude Code purges JSONL transcripts after about 30 days, so the SQLite DB is the only durable record. Three design choices follow:

1. **Capture before purge.** The `Stop` hook (`persist-session.sh`) writes each session to the DB after every completed turn, while the JSONL still exists, and a `SessionEnd` hook (`persist-on-exit.sh`) writes it once more when it closes, so a last turn interrupted before `Stop` fired is still counted. A throttled `SessionStart` hook (`safety-rescan.sh`) re-runs `backfill.sh` once a day in the background to catch any session both hooks missed (crash, kill, hook disabled), and to refresh a row whose transcript was written more than a minute after the row was, as long as its transcript is still within the 30-day window. The only unavoidable gaps are history older than the install date and downtime longer than 30 days.

2. **Store raw tokens, derive on demand.** Each row stores the raw token breakdown: `input_tokens` (regular input + cache write), `cache_creation_tokens` (cache write), `cache_creation_1h_tokens` (the subset of that write made at the 1-hour TTL), `cache_read_tokens`, and `output_tokens`, plus `output_context_sum`, the sum over the session's assistant messages of `output_tokens x (input + cache_write + cache_read)`, the KV-cache size each generated token re-reads. No formula uses that last column yet (see Cache read energy below); it is captured so a context-dependent decode term can be applied to history later. Cost and CO2 are pure functions of these counts plus `data/factors.json` and `data/prices.json`, so they can be regenerated at any time with `recompute.sh` without re-reading the (purged) JSONL. When a CO2 factor is revised, run `recompute.sh` (CO2-only by default); when a price changes, run `recompute.sh --with-cost`. Rows are tagged `methodology_version`; only version >= 2 carries the full raw-token breakdown, so older rows captured before this change are left untouched as legacy. The installers and `update.sh` run `recompute.sh` for every user, and that gate is what keeps legacy rows as they were stored.

3. **Store the split by model.** Since September 2026 each session also stores one `session_models` row per model, with the same token columns. For every session that has these rows, each token column, `co2_grams` and `cost_usd` of the session equal the sum over them, and `recompute.sh` re-derives each row at its own model and sets the session to the sum. What is exact and what is not, after a recompute:

| Rows | What recompute has | Result |
|---|---|---|
...
The `cost_usd` column is the theoretical API list value of the usage (what it would cost on pay-as-you-go), not the subscription price actually paid. It uses current Anthropic list pricing per million tokens (platform.claude.com/docs/en/about-claude/pricing, re-read 2026-09-23), per model where a model's price differs from its family's, per family otherwise:

| Model | Input | Output | Cache write 5 min | Cache write 1 h | Cache read |
|---|---|---|---|---|---|
| Opus 5.5 (`claude-opus-5-5`) | $4 | $20 | $5 | $8 | $0.20 (0.05x) |
| Opus 5, 4.8, 4.7, 4.6 (Opus family) | $5 | $25 | $6.25 | $10 | $0.50 |
| Fable 5.1, Mythos 5.1 (`claude-fable-5-1`, `claude-mythos-5-1`) | $10 | $50 | $12.50 | $20 | $0.25 (0.025x) |
| Fable 5, Mythos 5 (Fable family) | $10 | $50 | $12.50 | $20 | $1 |
| Sonnet 5 (Sonnet family) | $2 | $10 | $2.50 | $4 | $0.20 |
| Sonnet 4.6, 4.5, 4, 3.7, 3.5 | $3 | $15 | $3.75 | $6 | $0.30 |
| Haiku 4.5 (Haiku family) | $1 | $5 | $1.25 | $2 | $0.10 |

The per-model prices live in `model_overrides` in `data/prices.json`. A key matches a model id exactly or followed only by a dated snapshot suffix (`-YYYYMMDD`), so `claude-opus-5-5` covers `claude-opus-5-5-20260922` but never a hypothetical `claude-opus-5-50`; the longest matching key wins. Before September 2026 prices were per family only: a Sonnet 4.6 session was under-billed by a third, Fable 5.1 cache reads were over-billed 4x, and Opus 5.5 (released 2026-09-22) was billed at the Opus 5 price. Emission factors stay per family: a model's energy is not measured, and a lower price is not evidence of lower energy, so Opus 5.5 uses the Opus factors. Cache writes are billed per TTL tier, 1.25x input for the 5-minute tier and 2x for the 1-hour tier, and the tier is not assumed: each session's split is read from `usage.cache_creation.ephemeral_1h_input_tokens` and stored alongside the total, so a session mixing both tiers is priced correctly.
...
| Context length at decode | Not in the formula | Decode energy per generated token rises 2.26x from 1k to 16k context on a GQA model (Ma et al., arXiv:2605.11999), and ~3x from 2k to 10k (TokenPowerBench). Average request energy per output token, amortised prefill included, goes from 548 to 2,358 mJ as the output grows from 128 to 8,192 tokens on a fixed 512-token prompt (Llama-3.1-8B, H200, batch 16; Vellaisamy et al., IISWC 2026, arXiv:2608.28044, Table V), a 4.3x ratio computed here, not stated in the paper; only sliding-window and state-space architectures stay flat (Chkir et al., arXiv:2608.25096). Nothing is measured past 16k tokens, and Claude's attention architecture is unpublished. | x1.26 to x5.64, for a term of the form output x (1 + context / C0) with C0 from 400k down to 13k tokens, C0 being the context at which KV-cache reads equal weight reads | Not modelled; `output_context_sum` stored per session since September 2026 so a calibrated term can be applied to history |
...
**Convergence on real logs.** Zeke Hausfather (The Climate Brink, August 2026) applied three per-token methods to eight weeks of his own Claude Code logs, 3.2 billion tokens, and found that all three land between roughly 70 and 330 kWh over the period, which he reads as uncertain by a factor of ~2 in either direction, with a median session around 600 Wh. Two of the three, Watershed's activity tier and Couch's price-scaled figures, are discussed above. The third is a homonymous project (metztim/claude-carbon, price-inferred coefficients), not this tool. His 96% cache-read share and his 10% cache assumption match this tool's data and default.
